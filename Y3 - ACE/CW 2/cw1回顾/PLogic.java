...
public class PLogic {
    ...
    public static void main(String[] args) {
        ...
        if (!isLegal(formula0)){
            System.out.print("Error, you should enter legal propositional logic formula.");
            return;
        }
        ...
        if (!isNumeric(numStr)){
            System.out.print("Error, you should enter an integer as the total number of propositional variables.");
            return;
        }
        int num = Integer.parseInt(numStr); // (Integer)total number of propositional variables
        if(num<0||num>200){
            System.out.print("The total number of propositional variables should be 0~200.");
            return;
        }else if(num>0&&num<=200){
            Boolean[] truthValue = new Boolean[num]; // truth value of propositional variable
            if(num!=arr.length){
                System.out.print("Please ensure the number you've entered equals to the number of propositional variables.");
                return;
            }
            for (int i = 0; i < num; i++) {
                String truthValueStr = scanner.next();
                if(!isBoolean(truthValueStr)){
                    System.out.print("Error, you should enter boolean.");
                    return;
                }else{
    ...
    private static boolean isLegal(String s) { //propositional logic formula in legal form
        for (int i = 0; i < s.length(); i++) { 
            char ch = s.charAt(i);
            ...
            if((i > 0)&&(i < (s.length()))){ //if i>0,i<length, (a-z,T,F) near to (a-z,T,F),return false
                if(((ch >= 'a' && ch <= 'z')||(ch == 'T')||(ch == 'F'))&&((s.charAt(i-1) >= 'a' && s.charAt(i-1) <= 'z')||s.charAt(i-1) == 'T'||s.charAt(i-1) == 'F')){
                    return false;
                }
            }
            if(i > 0 && i < (s.length()-1)){ //if i>0,i<length-1, char near to char, return false
